...
vm.runInContext(src.slice(0,src.indexOf('let SETTINGS=await firstLanguage(loadSettings());'))+
  '\nglobalThis.api={merge,DEFAULTS,TABLE_PROFILES,layoutAutoEnabled,widgetAvailableWidth,effectiveMatchLayout,effectiveListLayout,tableAvailableWidth,tableSlotLayout,tableStatWidth,standingValue,tableStatCell,table,html};',ctx);
const a=ctx.api;
const rows=[
...
assert.ok(html.includes("setLayoutAuto('medium','lists')"));
assert.ok(html.includes("setLayoutAuto('large','match')"));
assert.ok(html.includes("setLayoutAuto('large','lists')"));
assert.ok(html.includes("setLayoutAuto('large','table')"));
...
for(const sportId of Object.keys(a.TABLE_PROFILES)){
  ...
  assert.equal(s.sportId,sportId,`selected ${sportId} must survive merge`);
  assert.equal(a.layoutAutoEnabled(s,'small','match'),true);
  assert.equal(a.layoutAutoEnabled(s,'medium','match'),true);
  assert.equal(a.layoutAutoEnabled(s,'medium','lists'),true);
  assert.equal(a.layoutAutoEnabled(s,'large','match'),true);
  assert.equal(a.layoutAutoEnabled(s,'large','lists'),true);
  assert.equal(a.layoutAutoEnabled(s,'large','table'),true);
}
const football=a.merge({...a.DEFAULTS,sportId:'football',leagueId:'cze.1'});
const basketball=a.merge({...a.DEFAULTS,sportId:'basketball',leagueId:'nba'});
const footballEvent={state:'post',completed:true,date:new Date().toISOString(),home:{score:'2'},away:{score:'1'}};
const basketballEvent={state:'post',completed:true,date:new Date().toISOString(),home:{score:'115'},away:{score:'108'}};
...
assert.ok(html.includes('smallMatchModeBadge')&&html.includes('mediumListsModeBadge')&&html.includes('largeListsModeBadge'));
(async()=>{
  for(const sportId of Object.keys(a.TABLE_PROFILES)){
