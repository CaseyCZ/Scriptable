...
# The JS source stores HTML quotes escaped once (\"). Normalize the Python search accordingly.
if layout_screen not in src:
    layout_screen = '<div class=\"screen\" id=\"layout\">${back}'
if layout_screen not in src:
    raise SystemExit('wallpaper screen: layout anchor not found')
...
if '\\\"' not in layout_screen:
    wallpaper_screen = wallpaper_screen.replace('\\\\\"','\\\"')
src = src.replace(layout_screen, wallpaper_screen + layout_screen, 1)
